...
// Settings
enum { NUM_TASKS = 10 };           // Number of tasks (philosophers)
...
static SemaphoreHandle_t eating_sem;
static SemaphoreHandle_t available_slots;  // Notifies main task when done
static SemaphoreHandle_t chopstick[NUM_TASKS];
static bool eating[NUM_TASKS];
...
void eat(void *parameters) {
  ...
  while (1) {
    xSemaphoreTake(eating_sem, portMAX_DELAY);
    if ((!eating[(num+1)%NUM_TASKS]) && (!eating[(num-1)%NUM_TASKS])) {
      eating[num] = true;
      xSemaphoreGive(eating_sem);
      break;
    }
    xSemaphoreGive(eating_sem);
  }
...
void setup() {
  ...
  available_slots = xSemaphoreCreateCounting(NUM_TASKS/2, NUM_TASKS/2);
  eating_sem = xSemaphoreCreateMutex();
