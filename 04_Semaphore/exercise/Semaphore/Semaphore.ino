...
static SemaphoreHandle_t bin_head_mutex;     // Waits for head
static SemaphoreHandle_t bin_tail_mutex;     // Waits for tail
static SemaphoreHandle_t occupied_slot_sem;
static SemaphoreHandle_t empty_slot_sem;
...
void producer(void *parameters) {
  ...
  for (int i = 0; i < num_writes; i++) {
    ...
    if (xSemaphoreTake(empty_slot_sem, portMAX_DELAY) == pdTRUE) {
      if (xSemaphoreTake(bin_head_mutex, portMAX_DELAY) == pdTRUE) {
        buf[head] = num;
        head = (head + 1) % BUF_SIZE;
        xSemaphoreGive(bin_head_mutex);
      }
      xSemaphoreGive(occupied_slot_sem);
    }
  }
...
void consumer(void *parameters) {
  ...
  while (1) {
    ...
    if (xSemaphoreTake(occupied_slot_sem, portMAX_DELAY) == pdTRUE) {
      if (xSemaphoreTake(bin_tail_mutex, portMAX_DELAY) == pdTRUE) {
        val = buf[tail];
        tail = (tail + 1) % BUF_SIZE;
        Serial.println(val);
        xSemaphoreGive(bin_tail_mutex);
      }
      xSemaphoreGive(empty_slot_sem);
    }
  }
}
...
void setup() {
  ...
  bin_head_mutex = xSemaphoreCreateMutex();
  bin_tail_mutex = xSemaphoreCreateMutex();
  occupied_slot_sem = xSemaphoreCreateCounting(BUF_SIZE, 0);
  empty_slot_sem = xSemaphoreCreateCounting(BUF_SIZE, BUF_SIZE);
